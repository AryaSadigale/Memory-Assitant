[README (2).md](https://github.com/user-attachments/files/26459475/README.2.md)
...
Install [Docker Desktop for Mac](https://www.docker.com/products/docker-desktop/) and make sure it is running before proceeding.
...
cd Memory-Assistant/knowledge-graph-rag
docker compose build app
docker compose up -d neo4j
```
...
To persist the key across terminal sessions, add the line above to your shell profile:
...
# For bash
echo 'export GROQ_API_KEY="your_actual_key"' >> ~/.bash_profile
source ~/.bash_profile
```
...
You should see the `neo4j` service listed as `running`. The Neo4j browser is available at `http://localhost:7474` (default credentials: `neo4j` / `neo4j`).
...
To also remove Neo4j data volumes:
...
> **Note:** iOS does not support running Docker containers natively. To use Memory-Assistant on an iOS device, connect remotely to a Mac or Linux machine that is running the stack.

#### Option 1 — SSH into a Remote Host

Install a terminal app such as [Termius](https://termius.com/) or [Blink Shell](https://blink.sh/), then SSH into your Mac or server:

```bash
ssh user@your-mac-hostname.local
cd Memory-Assistant/knowledge-graph-rag
docker compose run --rm app python main.py
```

#### Option 2 — Use a Cloud VM

Spin up a cloud instance (AWS EC2, Google Cloud, DigitalOcean, etc.), deploy the stack there, then SSH into it from your iOS terminal app using the same commands above.

#### Option 3 — Web Interface (Planned)

If a web front-end is added to the project in a future release, it can be accessed directly from Mobile Safari or any iOS browser without needing SSH.
